...
async def test_agent_run_streams_tokens() -> None:
    ...
    with TestClient(app) as client:
        with client.stream(
            "POST", "/agent/run", json={"thread_id": "thread-1", "prompt": "say hi"}
        ) as resp:
...
async def test_agent_run_respects_smaller_client_token_budget() -> None:
    ...
    with TestClient(app) as client:
        with client.stream(
            "POST",
            "/agent/run",
            json={"thread_id": "thread-3", "prompt": "say hi", "max_tokens": 120},
        ) as resp:
            assert resp.status_code == 200
            body = b"".join(resp.iter_bytes()).decode("utf-8")
...
async def test_agent_run_uses_nebius_price_lookup_for_cost() -> None:
    ...
    with TestClient(app) as client:
        with client.stream(
            "POST", "/agent/run", json={"thread_id": "thread-5", "prompt": "say hi"}
        ) as resp:
            assert resp.status_code == 200
            body = b"".join(resp.iter_bytes()).decode("utf-8")
...
async def test_agent_run_reuses_stored_thread_memory() -> None:
    ...
    with TestClient(app) as client:
        with client.stream(
            "POST",
            "/agent/run",
            json={"thread_id": "reader-1", "prompt": "My favorite author is Ursula Le Guin."},
        ) as resp:
            assert resp.status_code == 200
        ...
        with client.stream(
            "POST",
            "/agent/run",
            json={"thread_id": "reader-1", "prompt": "What did I say my favorite author was?"},
        ) as resp:
            assert resp.status_code == 200
