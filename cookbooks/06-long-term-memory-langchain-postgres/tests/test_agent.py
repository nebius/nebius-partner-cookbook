...
@pytest.mark.asyncio
@respx.mock
async def test_agent_run_recalls_long_term_memory_across_threads() -> None:
    nebius_route = _mock_nebius_stream()
    ...
    with TestClient(app) as client:
        ...
        memories = client.get("/memory/reader-2")
        summary = client.get("/memory/reader-2/summary")
        ...
        delete_resp = client.delete("/memory/reader-2")

    assert '"phase": "long_term_memory_saved"' in first_body
    assert memories.status_code == 200
    assert memories.json()["memories"][0]["text"] == "concise science fiction recommendations"
    assert summary.status_code == 200
    assert summary.json()["memoryCount"] == 1
    assert "concise science fiction" in summary.json()["summary"]
    assert '"longTermMemories": 1' in second_body
    assert delete_resp.json() == {"userId": "reader-2", "deleted": 1}
