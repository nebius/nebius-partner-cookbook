...
class NebiusPricing:
    ...
    def get_prices(self) -> TokenPrices:
        ...
        try:
            ...
            model = self._find_model(catalog, self.settings.nebius_model)
            input_price = self._first_price(model, ("input", "prompt"))
            output_price = self._first_price(model, ("output", "completion", "generated"))
    ...
    def _first_price(self, model: dict[str, Any], key_hints: tuple[str, ...]) -> float:
        for path, value in self._walk(model):
            ...
            if not any(hint in path_text for hint in key_hints):
                continue
