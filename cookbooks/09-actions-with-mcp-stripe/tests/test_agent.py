...
def _event_data(body: str, event_name: str) -> dict[str, object]:
...
@pytest.mark.asyncio
@respx.mock
async def test_purchase_prompt_creates_approval_without_nebius_or_stripe_call() -> None:
    nebius_route = _mock_nebius_stream()
    stripe_route = respx.post("https://mcp.stripe.com").mock(
    ...
    with TestClient(app) as client:
        with client.stream(
            ...
            json={
                "thread_id": "actions-1",
                "user_id": "reader-1",
                "prompt": "I want to buy the book The Nebius Cloud Atlas.",
            },
        ) as resp:
            assert resp.status_code == 200
            body = b"".join(resp.iter_bytes()).decode("utf-8")

    approval = _event_data(body, "approval_required")
    assert approval["action"] == "stripe.create_payment_link"
    assert approval["book"] == {
    ...
    assert nebius_route.calls.call_count == 0
    assert stripe_route.calls.call_count == 0


@pytest.mark.asyncio
@respx.mock
async def test_rejected_approval_never_calls_stripe_mcp() -> None:
    _mock_nebius_stream()
    stripe_route = respx.post("https://mcp.stripe.com").mock(
        return_value=httpx.Response(200, json={})
    )
    ...
    with TestClient(app) as client:
        with client.stream(
            ...
            json={
                "thread_id": "actions-2",
                "user_id": "reader-1",
                "prompt": "Please create a payment link for The Nebius Cloud Atlas book.",
            },
        ) as resp:
            body = b"".join(resp.iter_bytes()).decode("utf-8")
        approval_id = str(_event_data(body, "approval_required")["approvalId"])
        reject = client.post(f"/approvals/{approval_id}", json={"decision": "reject"})

    assert reject.status_code == 200
    assert reject.json()["status"] == "rejected"
    assert reject.json()["checkoutUrl"] is None
    assert stripe_route.calls.call_count == 0


@pytest.mark.asyncio
@respx.mock
async def test_approved_approval_calls_stripe_mcp_and_is_idempotent() -> None:
    _mock_nebius_stream()
    stripe_route = respx.post("https://mcp.stripe.com").mock(
        return_value=httpx.Response(
            200,
    ...
    with TestClient(app) as client:
        with client.stream(
            ...
            json={
                "thread_id": "actions-3",
                "user_id": "reader-1",
                "prompt": "Buy The Nebius Cloud Atlas book for me.",
            },
        ) as resp:
            body = b"".join(resp.iter_bytes()).decode("utf-8")
        approval_id = str(_event_data(body, "approval_required")["approvalId"])
        approved = client.post(f"/approvals/{approval_id}", json={"decision": "approve"})
        second = client.post(f"/approvals/{approval_id}", json={"decision": "approve"})

    assert approved.status_code == 200
    assert approved.json()["status"] == "completed"
    assert approved.json()["checkoutUrl"] == "https://buy.stripe.com/test_123"
    assert approved.json()["stripePaymentLinkId"] == "plink_test_123"
    assert second.status_code == 200
    assert second.json()["checkoutUrl"] == "https://buy.stripe.com/test_123"
    assert stripe_route.calls.call_count == 1

    sent_body = json.loads(stripe_route.calls.last.request.content)
    assert sent_body["method"] == "tools/call"
    assert sent_body["params"]["name"] == "create_payment_link"
    assert sent_body["params"]["arguments"]["line_items"] == [
        {"price": "price_test_nebius", "quantity": 1}
    ]


@pytest.mark.asyncio
@respx.mock
async def test_expired_approval_fails_closed() -> None:
    _mock_nebius_stream()
    ...
    with TestClient(app) as client:
        with client.stream(
            ...
            json={
                "thread_id": "actions-4",
                "user_id": "reader-1",
                "prompt": "Buy The Nebius Cloud Atlas book.",
            },
        ) as resp:
            body = b"".join(resp.iter_bytes()).decode("utf-8")
        approval_id = str(_event_data(body, "approval_required")["approvalId"])
        ...
        record = approvals.get_approval_store().get(approval_id)
    ...
    assert approved.json()["detail"] == "approval expired"
