...
+-----------------------------------+
    ...
    |         |
    |         v
    |    Returns SopAuditResult(findings, tokens, status) — aggregated by
    |    value per audit call (no shared global; concurrency-safe). Transient
    |    failures retried per SOP (status-based, up to 4 attempts). Emits a
    |    token line parsed by the UI + validate_run via token_accounting.py
    |
...
Three screens: **Audit** (composer + agent picker + live stream + Jira findings
register), **Compare** (3-way agent race on one question), **Eval** (120-question
benchmark dashboard). A running audit can be stopped from the composer — Stop
aborts the stream and the server cancels the LangGraph run, so the spend actually
stops. Agent labels and pricing come from `/agents.js`, generated by the server
from `config.PRICING` — change prices in one place.
...
`audit_all_sops` fans out sub-agents through a `ThreadPoolExecutor` (configurable via `MAX_AUDIT_WORKERS`). Transient failures (e.g. provider 504s) are retried per SOP — up to 4 attempts with backoff, longer on rate limits — based on a structured status carried on each result, with tokens from failed attempts still counted. Retrieval is capped at 30 calls per sub-agent.
...
│   │   ├── naive_agent.py     # Act 0 baseline: single retrieval + single LLM call
│   │   └── tools.py           # LangChain tools: sub-agent auditing + retrieval
│   ├── retrieval/
│   │   ├── local.py           # SOP file loading and search (cached parse)
│   │   ├── regulations.py     # Pinecone regulation text retrieval
│   │   ├── ingest.py          # SOP -> Pinecone ingestion + shared section chunker
│   │   └── ingest_regulations.py  # Regulation text -> Pinecone ingestion
│   ├── eval/
│   │   └── metrics.py         # Confusion-matrix / F1 scoring (single implementation)
│   ├── actuation/
│   │   └── jira_client.py     # Jira Cloud REST client
├── ui/
│   ├── server.py              # FastAPI backend: agent registry, SSE streaming, Jira findings
│   └── static/                # React screens (Audit / Compare / Eval), served by server.py
├── scripts/
│   ├── validate_run.py        # Audit quality evaluation against compliance matrix
│   ├── compare_audit_runs.py  # Side-by-side comparison of N audit runs
│   ├── run_qa_eval.py         # 120-question Q&A eval (naive vs agentic modes)
...
├── tests/                     # 150 regression tests (no API keys needed)
├── data/
...
# Compare 3+ runs (quality, cost, tokens, latency, tool-call counts)
python3 scripts/compare_audit_runs.py <run-id-1> <run-id-2> <run-id-3>
```
...
| `NEBIUS_MODEL` | Optional | Model for the Optimized agent (`deepseek-v4-pro` default, `nemotron`, `kimi-k2`, `glm-5`); unknown values fail fast at startup |
| `MAX_AUDIT_WORKERS` | Optional | Sub-agent fan-out width for `audit_sops` / `audit_all_sops` (default 10) |
...
| `LANGCHAIN_PROJECT` | Optional | LangSmith project name used by the eval scripts (default `sentinel-agent`) |
...
| `JIRA_DEFAULT_ISSUE_TYPE` | Optional | Jira issue type for filed tickets (default `Task`) |
| `LANGGRAPH_URL` | Optional | LangGraph server the UI talks to (default `http://localhost:2024`) |
