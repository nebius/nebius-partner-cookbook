...
// Agent labels/models/pricing come from /agents.js (window.SENTINEL_AGENTS),
// generated by ui/server.py from one registry + config.PRICING.
const AGENT_PRICING = (window.SENTINEL_AGENTS || {}).pricing || {};
const AGENT_CONFIG = Object.fromEntries(
  (((window.SENTINEL_AGENTS || {}).race) || []).map(a => [a.key, a]),
);
...
const CompareScreen = () => {
  ...
  const renderAgents = ["naive", "openai", "nemotron"].map(key => {
    ...
    return {
      ...
      stream: [
        ...a.toolCalls.map(tc => ({ kind: "tool", name: tc.name, arg: summarizeArgs(tc.args), result: tc.result ? truncate(tc.result, 220) : undefined, status: tc.result ? undefined : "running" })),
...
// truncate / summarizeArgs / TraceLink come from primitives.jsx
  ...
  return (
    <Slab padding={0} style={{ display: "flex", flexDirection: "column" }}>
      ...
      <div style={{
        ...
        {a.key === "nemotron" && <TraceLink url={a.traceUrl}/>}
      </div>
